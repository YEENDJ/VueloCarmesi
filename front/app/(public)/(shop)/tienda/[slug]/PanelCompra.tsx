...
export default function PanelCompra({ producto }: { producto: Producto }) {
  ...
  if (sinStock) {
    return (
      <div>
        <p className="ficha-compra-estado ficha-compra-estado--agotado">Sin stock por ahora</p>
        <Button disabled style={ESTILO_BOTON}>Agregar al carrito</Button>
        <p className="ficha-compra-nota">
          Escríbenos por el formulario de contacto y te avisamos cuando vuelva.
        </p>
      </div>
    )
  }
      ...
      <div ref={accionesRef}>
        {pocoStock ? (
          <div className="ficha-compra-alerta">
            ...
            <span style={{ minWidth: 0 }}>Quedan solo {producto.stock} unidades</span>
          </div>
        ) : (
          <p className="ficha-compra-estado">Stock disponible</p>
        )}

        <div className="ficha-compra-cantidad">
          <span className="ficha-compra-cantidad-rotulo">Cantidad</span>
          <QuantitySelector value={cantidad} onChange={setCantidad} min={1} max={producto.stock} />
          <span className="ficha-compra-disponibles">({producto.stock} disponibles)</span>
        </div>

        <div className="ficha-compra-acciones">
          <Button onClick={comprarAhora} style={ESTILO_BOTON}>Comprar ahora</Button>
          <Button onClick={agregar} variant="outline" style={ESTILO_BOTON}>Agregar al carrito</Button>
        </div>

        {cantidad > 1 && (
          <p className="ficha-compra-subtotal">
            Subtotal {cantidad} unidades · <strong>{formatPrecio(subtotal)}</strong>
          </p>
        )}
      </div>
      ...
      {barraVisible && (
        <div className="ficha-prod-barra">
          <div className="ficha-prod-barra-precio">
            <span className="ficha-prod-barra-cifra">{formatPrecio(subtotal)}</span>
            {cantidad > 1 && <span className="ficha-prod-barra-detalle">{cantidad} unidades</span>}
          </div>
          <Button onClick={agregar} style={{ flexShrink: 0 }}>Agregar al carrito</Button>
        </div>
      )}
    </>
  )
