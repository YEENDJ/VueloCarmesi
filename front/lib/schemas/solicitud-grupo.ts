...
const TELEFONO_REGEX = /^\+?(?:[\s-]*\d){7,15}[\s-]*$/

/** Mañana, en zona horaria de Colombia, como `YYYY-MM-DD` para el `min` del input. */
export function fechaMinima(): string {
  const manana = new Date()
  manana.setDate(manana.getDate() + 1)
  return manana.toLocaleDateString('en-CA', { timeZone: 'America/Bogota' })
}
...
export const solicitudGrupoSchema = z.object({
  ...
  contacto: z.string().trim().min(3, 'errorContacto').max(100, 'errorContacto'),
  ...
  requiereTransporte: z.boolean().optional(),
